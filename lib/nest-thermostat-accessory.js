...
class NestThermostatAccessory extends NestDeviceAccessory {
    constructor(conn, log, device, structure, platform) {
       ...
       */
        ...
        // Exposed so threshold getters can clamp to HomeKit characteristic bounds.
        this.minSetTemp = minSetTemp;
        this.maxSetTemp = maxSetTemp;
    ...
    // Clamp to the configured setpoint bounds. Nest reports 0 (or omits the
    // field entirely) for an inactive threshold — e.g. the heating threshold
    // when the device is in cool-only mode — and HAP-NodeJS rejects values
    // outside the characteristic's minValue/maxValue with a warning that
    // shows up in the Homebridge log.
    clampSetTemp(value) {
        if (!Number.isFinite(value) || value < this.minSetTemp) {
            return this.minSetTemp;
        }
        if (value > this.maxSetTemp) {
            return this.maxSetTemp;
        }
        return value;
    }
    ...
    getCoolingThresholdTemperature() {
        ...
        if (this.isZirconium || this.device.hvac_mode != 'eco') {
            value = this.unroundTemperature(this.device.target_temperature_high || this.device.target_temperature);
        } else {
            value = this.unroundTemperature(this.device.eco_temperature_high || this.device.away_temperature_high);
        }
        return this.clampSetTemp(value);
    }

    getHeatingThresholdTemperature() {
        let value;
        if (this.isZirconium || this.device.hvac_mode != 'eco') {
            value = this.unroundTemperature(this.device.target_temperature_low || this.device.target_temperature);
        } else {
            value = this.unroundTemperature(this.device.eco_temperature_low || this.device.away_temperature_low);
        }
        return this.clampSetTemp(value);
    }
