...
class Connection {
        ...
        while (true) {
            ...
            if (this.config.googleAuth || this.config.refreshToken) {
                ...
                try {
                    ...
                    this.error('Auth error: ' + redactedErrorDetail(error));
    ...
    protobufSendCommand(cmd, device_id, successResponse) {
        ...
        return Promise.resolve(axios({
            ...
            this.verbose('--> Command error: ' + (typeof err === 'string' ? err : redactedErrorDetail(err)));
            throw(err);
        });
    }
    ...
    pushUpdates(data) {
        ...
        return Promise.delay(additionalDelay).then(() => {
            ...
            this.verbose('Push updates error: ' + redactedErrorDetail(error));
...
// Summarise an error (typically an AxiosError) for logging. Deliberately omits
// error.config, error.request, error.response.headers and error.response.data, as
// these carry bearer tokens, Google cookies, API keys and JWTs.
function redactedErrorDetail(error) {
    ...
    let status = error.status || (error.response && error.response.status);
    if (status) {
        parts.push('status=' + status);
    }

    return parts.length ? parts.join(', ') : 'no further detail available';
}
