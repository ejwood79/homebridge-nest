...
NestDeviceAccessory.prototype.updateData = function (device, structure) {
    ...
    this.boundCharacteristics.map(function (c) {
        const ch = c[0].getCharacteristic(c[1]);
        // handleGetRequest() invokes the registered 'get' handler and emits
        // 'change' events, like the removed getValue(). Works on HAP-NodeJS
        // v0.14.x (Homebridge 1) and v1.0+ (Homebridge 2).
        if (typeof ch.handleGetRequest === 'function') {
            ch.handleGetRequest().catch(() => {});
        } else if (typeof ch.getValue === 'function') {
            ch.getValue();
        }
    });
};
